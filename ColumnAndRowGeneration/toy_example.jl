...
### Solve the despatching problem : generators meet the demand
println("---------------------------------------------------------------------------------------");
println("Unit Commitment Problem");
(p_matching, pbar_matching, u_matching, v_matching, w_matching, l_matching, obj_matching, _) = Matching(
    MinRunCapacity,
    MaxRunCapacity,
    RampUp,
    RampDown,
    ...
    pbar_posterior,
    true);
println("p = $(p_matching)"); # production for each generator at each ti,e
println("pbar_UC = $(pbar_matching)"); # production upper bound
println("u_UC = $(u_matching)"); # bin var : generator is on
println("v_UC = $(v_matching)"); # bin var : generqtor is off
println("w_UC = $(w_matching)"); # bin var : shut-down decision 
println("l_UC = $(l_matching)"); # demand to be meet by the generators
